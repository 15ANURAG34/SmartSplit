...
export default function AccountabilityCircle() {
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      ...
      const loaded = data.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
  ...
  const addFriend = async () => {
    ...
    const newFriend = { name, weeklyBudget: budget, weeklySpending: spending, progress };
    const docRef = await addDoc(friendsCollection, newFriend);
    setCircleData([...circleData, { ...newFriend, id: docRef.id }]);
    setNameInput("");
  };

  const deleteFriend = async (i) => {
    const friend = circleData[i];
    if (friend.id) {
...
`;
    ...
    try {
      ...
      const summaryRef = doc(db, "summary", "weekly");
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "2rem" }}>
        {circleData.map((member, idx) => {
          ...
          return (
            <div key={idx} style={{ background: "#fff", border: "1px solid #ddd", borderRadius: "10px", padding: "1rem", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    style={{ background: "#ef4444", color: "#fff", border: "none", fontSize: "0.75rem", padding: "4px 8px", borderRadius: "4px", cursor: "pointer" }}
                  >
                    Delete
                  </button>
                </div>
              </div>
              <div style={{ height: "8px", backgroundColor: "#e5e7eb", borderRadius: "999px", overflow: "hidden", marginBottom: "0.5rem" }}>
                <div style={{ width: `${Math.min(progress, 100)}%`, height: "100%", backgroundColor: color, transition: "width 0.3s ease" }} />
              </div>
              <div style={{ fontSize: "0.9rem", color: "#374151", textAlign: "right", fontWeight: "500" }}>
      ...
      <button
        ...
        disabled={loading}
        style={{ width: "100%", padding: "0.75rem", backgroundColor: loading ? "#b0a7f7" : "rgba(113, 99, 186, 1)", color: "#fff", fontWeight: "600", borderRadius: "6px", border: "none", cursor: loading ? "not-allowed" : "pointer" }}
      >
        {loading ? "Generating Summary..." : "Get Weekly Summary"}
      </button>

      {loading && (
        <div style={{ marginTop: "1rem", fontSize: "1.2rem", textAlign: "center", color: "#715FDB" }}>
          ⏳ Generating advice...
        </div>
      )}

      {summary && !loading && (
        <div style={{ marginTop: "1.5rem", background: "#fff8dc", padding: "1rem", borderRadius: "10px", border: "1px solid #facc15", fontSize: "0.95rem", whiteSpace: "pre-wrap", marginBottom: "2rem" }}>
          {summary}
        </div>
      )}
    </div>
